...
SQL_FILE="$BACKUP_DIR/sql/imoveis.sql"
...
echo "[restore-imoveis] Parando serviço..."
stop_service

echo "[restore-imoveis] Limpando dados de imóveis..."
PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U "$DB_USER" -d "$DB_NAME" <<'SQL'
TRUNCATE TABLE imovel_fotos, documental_imovel, imoveis, categorias_imovel RESTART IDENTITY CASCADE;
SQL

echo "[restore-imoveis] Restaurando dados de imóveis..."
PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U "$DB_USER" -d "$DB_NAME" < "$SQL_FILE"

echo "[restore-imoveis] Substituindo storage atual..."
rm -rf "$STORAGE_TARGET"
cp -a "$STORAGE_BACKUP" "$STORAGE_TARGET"

echo "[restore-imoveis] Subindo serviço..."
start_service
...
echo "[restore-imoveis] Restore concluído a partir de: $BACKUP_INPUT"

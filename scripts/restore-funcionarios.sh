...
SQL_FILE="$BACKUP_DIR/sql/funcionarios.sql"
if [[ ! -f "$SQL_FILE" ]]; then
...
echo "[restore-funcionarios] Limpando funcionários e cargos atuais..."
PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U "$DB_USER" -d "$DB_NAME" <<'SQL'
TRUNCATE TABLE funcionarios, cargos_funcionario RESTART IDENTITY CASCADE;
SQL
echo "[restore-funcionarios] Restaurando cargos e funcionários..."
...
echo "[restore-funcionarios] Restore concluído a partir de: $BACKUP_INPUT"
